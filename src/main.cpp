...
uint256 hashGenesisBlock("0x52cb6e4e481ebdb259b99cb3a09dd2b86b1052763510191cdd0776ad69b5778e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Silkcoin: starting difficulty is 1 / 2^12
...
{
    // Silkcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Silkcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Silkcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Silkcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Silkcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Silkcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa646bb581f6c175607e569a6e506dc7d760840e3e523d5f62e1c1fd84630d6b7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "17/Aug/2022 silkcoin coded by John Rezai";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0488d739a55f42e704680803879e74f1f5fc3f489eaaa2ed01d36dc2cdf6316e9abbcb99510d381daf9635c5f845b00866df96e40bf932ed91c7d12268e360f50c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1660746556;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084636688;

        if (fTestNet)
        {
            block.nTime    = 1660746538;
            block.nNonce   = 385897271;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc4c7a76ddce2b512fb84bcc2ef53907030a4d0a7f09dd241de88df3b6bf6c65a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xb3, 0xda }; // Silkcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SilkcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SilkcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SilkcoinMiner(CWallet *pwallet)
{
    printf("SilkcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running SilkcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SilkcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SilkcoinMiner, pwallet));
}
